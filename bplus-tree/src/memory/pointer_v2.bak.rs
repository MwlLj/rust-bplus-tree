...
impl BPlusTree {
    ...
    */
        ...
        if len > self.size {
            ...
            */
            ...
            BPlusTree::populate_the_inode(&k
                , &mut leftNodePtr, &mut rightNodePtr, &mut leaf.index, &mut self.root, self.size, true);
    ...
    fn populate_the_inode(newKey: &str, newLeftNode: &mut *mut Node, newRightNode: &mut *mut Node, parent: &mut *mut IndexNode, root: &mut Node, size: usize, isLeaf: bool) {
        /*
        ...
        */
        ...
        match unsafe{parent.as_mut()} {
                ...
                if len > size {
                    ...
                    if !isLeaf {
                        for item in leftIndexNode.items.iter_mut() {
                            item.parent = leftIndexNodePtr;
                        }
                        for item in rightIndexNode.items.iter_mut() {
                            item.parent = rightIndexNodePtr;
                        }
                    } else {
                        /*
                                        ...
                                        match node {
                                            Node::Index(p) => {
                                                match unsafe{p.as_mut()} {
                                                    Some(n) => {
                                                        for it in n.items.iter_mut() {
                                                            it.parent = leftIndexNodePtr;
                                                        }
                                                    },
                                                    None => {
                                                    }
                    ...
                    */
                    ...
                    BPlusTree::populate_the_inode(&newIndexKeyClone, &mut leftIndexBoxPtr, &mut rightIndexBoxPtr, &mut indexItem.parent, root, size, true);
            ...
            None => {
                println!("populate parent is none ...");
                /*
